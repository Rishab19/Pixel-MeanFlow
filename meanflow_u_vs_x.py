...
class UPredMeanFlowLoss:
    """
    u-prediction variant of MeanFlow (contrast with x-prediction in Algorithm 1).

    The network directly predicts the average velocity u.
    Training target is the true velocity v = e - x (same target as x-pred).

    u_hat      = net(z_t, t, h)              [direct velocity prediction]
    v_inst     = net(z_t, t, 0)              [instantaneous: h=0 => r=t]
    u, dudt    = jvp(net_as_u_fn, ...)
    V          = u_hat + (t-r) * stopgrad(dudt)
    loss       = ||V - (e - x)||^2
    """
    ...
    def __call__(self, net: nn.Module, x: torch.Tensor) -> torch.Tensor:
        ...
        # u_fn: for u-prediction the network IS u directly
        def u_fn(z, r_, t_):
            h = t_ - r_
            return net(z, t_, h)

        # Instantaneous velocity: h=0 means r=t
        v_inst = u_fn(z_t, t, t)
